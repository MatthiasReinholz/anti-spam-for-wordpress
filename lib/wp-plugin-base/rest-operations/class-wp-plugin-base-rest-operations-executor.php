	...
	class WP_Plugin_Base_REST_Operations_Executor {
		...
		public static function execute( array $operation, WP_REST_Request $request ) {
			if ( empty( $operation['callback'] ) || ! is_callable( $operation['callback'] ) ) {
				return new WP_Error(
					...
					__( 'The REST operation callback is invalid.', 'anti-spam-for-wordpress' ),
					array( 'status' => 500 )
				);
			}
			...
			try {
				...
				return new WP_Error(
					...
					__( 'The REST operation failed to execute.', 'anti-spam-for-wordpress' ),
					array( 'status' => 500 )
				);
			}
		...
		private static function log_execution_failure( array $operation, Throwable $error ) {
			...
			$message      = sprintf(
				...
				__( 'REST operation %1$s threw an uncaught %2$s.', 'anti-spam-for-wordpress' ),
				$operation_id,
				$error_class
			);
