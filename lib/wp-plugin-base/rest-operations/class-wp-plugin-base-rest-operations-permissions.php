...
if ( ! class_exists( 'ASFW_WP_Plugin_Base_REST_Operations_Permissions' ) ) {
	/**
	 * Evaluates capability and scope constraints for operations.
	 *
	 * @since NEXT
	 */
	class ASFW_WP_Plugin_Base_REST_Operations_Permissions {
		/**
		...
		public static function check_operation( $plugin_slug, array $operation, WP_REST_Request $request ) {
			...
			if ( ! is_user_logged_in() ) {
				return new WP_Error(
					...
					__( 'Authentication is required for this operation.', 'anti-spam-for-wordpress' ),
					array( 'status' => 401 )
				);
			}
			...
			if ( ! empty( $operation['capability_callback'] ) && is_callable( $operation['capability_callback'] ) ) {
				try {
					...
					return new WP_Error(
						...
						__( 'The REST operation permission callback failed.', 'anti-spam-for-wordpress' ),
						array( 'status' => 500 )
					);
				}
				...
				return new WP_Error(
					...
					__( 'You are not allowed to execute this operation.', 'anti-spam-for-wordpress' ),
					array( 'status' => 403 )
				);
			}
		...
		private static function invalid_capability_configuration_error() {
			return new WP_Error(
				...
				__( 'The REST operation capability configuration is invalid.', 'anti-spam-for-wordpress' ),
				array( 'status' => 500 )
			);
		}
		...
		private static function log_capability_failure( array $operation, Throwable $error ) {
			...
			$message      = sprintf(
				...
				__( 'REST operation %1$s capability_callback threw an uncaught %2$s.', 'anti-spam-for-wordpress' ),
				$operation_id,
				$error_class
			);
			...
			foreach ( $required_scopes as $required_scope ) {
				...
				if ( ! self::has_scope( $required_scope, $granted_scopes ) ) {
					return new WP_Error(
						...
						__( 'You do not have the required operation scope.', 'anti-spam-for-wordpress' ),
						array( 'status' => 403 )
					);
				}
			}
		...
		private static function granted_scopes( $plugin_slug, array $operation, WP_REST_Request $request ) {
			...
			try {
				...
				return new WP_Error(
					...
					__( 'The REST operation scope filter failed.', 'anti-spam-for-wordpress' ),
					array( 'status' => 500 )
				);
			}
			...
			if ( ! is_array( $filtered_scopes ) ) {
				return new WP_Error(
					'wp_plugin_base_rest_scope_check_failed',
					__( 'The REST operation scope filter returned invalid data.', 'anti-spam-for-wordpress' ),
					array( 'status' => 500 )
				);
			}
		...
		private static function invalid_scope_configuration_error() {
			return new WP_Error(
				...
				__( 'The REST operation scope configuration is invalid.', 'anti-spam-for-wordpress' ),
				array( 'status' => 500 )
			);
		}
	}
