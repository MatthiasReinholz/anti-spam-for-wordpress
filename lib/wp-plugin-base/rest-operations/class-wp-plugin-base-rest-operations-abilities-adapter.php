...
if ( ! class_exists( 'ASFW_WP_Plugin_Base_REST_Operations_Abilities_Adapter' ) ) {
	/**
	 * Registers abilities for operations when the Abilities API is available.
	 *
	 * @since NEXT
	 */
	class ASFW_WP_Plugin_Base_REST_Operations_Abilities_Adapter {
		/**
			...
			wp_register_ability_category(
				...
				array(
					...
					'description' => sprintf(
						...
						__( 'Abilities exposed by %s.', 'anti-spam-for-wordpress' ),
						$category_label
					),
				)
			);
		...
		private static function register_operation( $plugin_slug, $category_slug, array $operation ) {
			...
			$args    = array(
				...
				'description'         => ! empty( $ability['description'] ) ? $ability['description'] : sprintf(
					...
					__( 'Executes the %s operation.', 'anti-spam-for-wordpress' ),
					$operation['id']
				),
				...
				'execute_callback'    => function ( $input = null ) use ( $operation ) {
					...
					return ASFW_WP_Plugin_Base_REST_Operations_Responses::unwrap(
			...
			if ( ( null === $registered || false === $registered || is_wp_error( $registered ) ) && function_exists( '_doing_it_wrong' ) ) {
				_doing_it_wrong(
					...
					sprintf(
						...
						esc_html__( 'Could not register operation ability %s. Check its manifest and category.', 'anti-spam-for-wordpress' ),
						esc_html( $name )
					),
					'1.9.0'
				);
		...
		private static function prepare_request( array $operation, $input ) {
			$prepared_input = ASFW_WP_Plugin_Base_REST_Operations_Input::prepare_input( $operation, $input );
