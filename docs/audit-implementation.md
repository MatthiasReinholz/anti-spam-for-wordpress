...
Local combined validation passes **499 PHP tests / 3,118 assertions**, including randomized order under an explicit 128 MiB memory limit, plus **16 admin/dependency cases**. Coding standards, static analysis, admin lint, security scans, dependency audits, translation generation and package validation pass. [Follow-up PR #102](https://github.com/MatthiasReinholz/anti-spam-for-wordpress/pull/102) passed all **16 hosted checks** on commit `2f7bae349c715c36d2f9b85b8b569b5c8a5d2236` before protected merge. The [CI](https://github.com/MatthiasReinholz/anti-spam-for-wordpress/actions/runs/36145806623), [browser](https://github.com/MatthiasReinholz/anti-spam-for-wordpress/actions/runs/36145806627) and [WordPress](https://github.com/MatthiasReinholz/anti-spam-for-wordpress/actions/runs/36145806631) runs confirm the full supported matrix. These corrections are included in [0.10.1](https://github.com/MatthiasReinholz/anti-spam-for-wordpress/releases/tag/0.10.1), prepared through [PR #103](https://github.com/MatthiasReinholz/anti-spam-for-wordpress/pull/103). Each versioned release independently requires the same protected checks and signed publication; its tag, package and provenance are the completion evidence.
...
## Released-version upgrade audit on 25 September 2026

The next review fetched `origin/main` and confirmed that the latest published release was **0.10.1**, commit `204de98e50eecf00565763f53ce3ae6e5f04a3c0`. Three independent reviewers examined historical settings/migrations, public and provider contracts, documentation, extensibility and ownership; root reviewed their findings and tested the actual published packages. The downloaded 0.10.1 ZIP matches the independently verified signed and attested release artifact, SHA-256 `16ef9a6506539f749973bfe7a51bdd6dbb732826ed6c777f5fa4feed65ecdcad`.

The review found no additional accidental public-interface or data-migration regression from **0.10.0 to 0.10.1**. An unconditional upgrade guarantee for all older users would be incorrect. PHP 8.0 became required in 0.3.2 and WordPress 6.4 in 0.6.2. The 0.10 boundary deliberately changes proxy-header selection, shared-IP quotas, outstanding proof storage and precedence of modern Bunny options over conflicting legacy values. Before 0.8, an unset native-comments placement also had a different default. These changes can affect legitimate submissions or remote enforcement without deleting any settings.

An executable old/new proxy comparison reproduced an existing `X-Real-IP` deployment resolving to its proxy address until the new header selector was set explicitly. A separate old/new settings comparison reproduced changed effective Bunny credentials, dry-run/failure policy and thresholds when modern and legacy values conflict. Restoring automatic header precedence or copying all stale legacy settings would undermine the intended security and administrator-choice guarantees. The operator must review the actual deployment configuration using the [upgrade checklist](upgrading.md).

| Priority | Action | Result |
| --- | --- | --- |
| P2 | Handle valid callable settings-field renderers without breaking the admin response | A pre-existing string cast, also present in 0.9.0, threw for closures/invokable objects and warned for callable arrays. Type inference now compares only string callback names and otherwise uses explicit field metadata. Nine full-payload cases verify callable registration, absence of renderer execution during REST reads, and preserved built-in inference. Original-code controls produced five errors. |
| P2 | Make older-version upgrade requirements visible to operators | Added a linked checklist covering platform floors, proxy selection, pooled quotas, legacy Bunny settings, stale forms/caches, commercial-provider checks, cron/multisite and recovery without uninstalling. Existing Bunny blocks require explicit remote revocation even after local protection is disabled. |
| P3 | Correct the extension reference and current validation documentation | Clarified the legacy settings registration hook and custom-identifier detection filter, added a complete schema-field example, and corrected the current admin test count. Historical audit counts remain unchanged. |

Real WordPress/MariaDB testing installed hash-checked published **0.4.8, 0.8.8, 0.9.0 and 0.10.0** packages, configured each, and replaced it in place with the published **0.10.1** ZIP. On WordPress 7.1.2 / PHP 8.3, all four paths preserved the selected settings, secret and historical event row, verified/upgraded the event schema, completed initialization, kept maintenance scheduled and accepted a fresh proof exactly once. Old transient-backed proofs were rejected; an outstanding 0.10.0 proof remained valid. This matrix exercises the main site of an isolated multisite environment; it is separate from the existing Redis/concurrency/network-lifecycle suites and does not certify every provider or historical state.

The callable correction passes the complete local quality pack: **508 PHP tests / 3,165 assertions**, coding standards and static analysis. Fresh root/admin dependency audits, including development dependencies, report zero known advisories. The repository has no open dependency, code-scanning or secret-scanning alerts at this dated checkpoint. Foundation **v1.9.0** remains the latest published upstream version and is unchanged by this follow-up.

The project has coherent service, provider-adapter and admin-component boundaries. Compatibility facades and the child/foundation ownership split are intentional. The review supports retaining that organization; its concrete improvements concern extension behavior and discoverable, accurate operational guidance. The audited 0.10.1 package contains the pre-existing callable-field defect; the correction from this review is separate from that package. Built-in settings do not use those problematic callback shapes.
