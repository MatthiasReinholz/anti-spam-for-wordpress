...
The final adopted tree passes the local checks below. Real WordPress and Firefox/WebKit evidence remains tied to the earlier combined candidate and hosted checkpoint until the new pull-request checks complete.
...
Hosted checkpoint `245a500e4f479281c6517b3f17e1c6c80cacdbbc` passed all **16 reported checks**, including **45 cases per browser engine**, both real WordPress versions and PHP 8.0–8.5. Fresh hosted checks on the final adoption commit remain pending, and the release commit must pass its own required checks. Local Firefox cannot start reliably in this macOS environment; hosted Linux CI supplies its browser evidence.
...
The refreshed child admin graph passes clean installation, build, lint, all 13 tests and a zero-advisory audit. Its React 18 pins, custom source and scoped overrides are retained. Foundation sync preserves child-owned dependency manifests and locks; matching starter changes require deliberate child review and regeneration. Foundation action pins belong to the upstream catalog, including reviewed migrations of custom workflows. The initial adoption from older update automation is performed manually so those migrated paths are included in the reviewed commit.
