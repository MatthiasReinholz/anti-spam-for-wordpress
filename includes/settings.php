...
if ( is_admin() ) {
	...
	function asfw_settings_init() {
		...
		asfw_register_setting_option(
			...
			function ( $value ) {
				return asfw_sanitize_enum_option( $value, array( 'ip', 'ip_ua' ), 'ip' );
			}
		);
		asfw_register_setting_option(
		...
		add_settings_field(
			...
			'asfw_security_settings_section',
			array(
				'name'    => AntiSpamForWordPressPlugin::$option_visitor_binding,
				'hint'    => __( 'Choose how challenges and rate limits identify a visitor. IP + User Agent reduces collisions on shared IPs but is more sensitive to browser changes.', 'anti-spam-for-wordpress' ),
				'options' => array(
		...
		add_settings_field(
			...
			'asfw_security_settings_section',
			array(
				'name' => AntiSpamForWordPressPlugin::$option_trusted_proxies,
				'hint' => __( 'Optional comma-separated IPs or CIDR ranges for reverse proxies. When a request comes from one of these proxies, the plugin will trust forwarded client IP headers.', 'anti-spam-for-wordpress' ),
				'type' => 'text',
			)
		);
